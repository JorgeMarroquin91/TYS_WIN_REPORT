...
{
    public partial class TysWinReportFrmConteoVehicularPorDiaPorHoraPorClase: Form
    {
        ...
        private List<ClasificacionVehicular> clasificacionVehiculars = new List<ClasificacionVehicular>();

        public TysWinReportFrmConteoVehicularPorDiaPorHoraPorClase()
        {
            InitializeComponent();
        }

        private void TysWinReportFrmConteoVehicularPorDiaPorHora_Load(object sender, EventArgs e)
        {
            this.listarEstacion();
            this.limitDate();
        ...
        private void listarEstacion()
        {
            ...
            this.clasificacionVehiculars.Clear();

            this.clasificacionVehiculars = (List<ClasificacionVehicular>)_clasificacionVehicularRepositoryQuery.GetAll();

            cbxClase.DataSource = this.clasificacionVehiculars;
            cbxClase.DisplayMember = "clasificacion";
            cbxClase.ValueMember = "idClasificacionVehicular";
        }

        private void btnGenerar_Click(object sender, EventArgs e)
        {
            var estacion = (Estacion)cbxEstacion.SelectedItem;
            var clasificacionVehicular = (ClasificacionVehicular)cbxClase.SelectedItem;
            ...
            try
            {
                ...
                parametros[18] = new ReportParameter("clase", clasificacionVehicular.clasificacion ?? "");

                var reportDataSource = new ReportDataSource(name: "DataSet1", dataSourceValue: (DataTable)(new SP_ConteoPorDiaYClasificacionTableAdapter().GetDataConteoVehicularPorDiaPorHoraPorClase(estacion.nombreEstacion + fecha.Year, fecha.ToString("yyyy/MM/dd"), clasificacionVehicular.clasificacion ?? "")));
        ...
        private void TysWinReportFrmConteoVehicularPorDiaPorHora_SizeChanged(object sender, EventArgs e)
        {
            reportViewer1.Width = this.Width - 60;
            reportViewer1.Height = this.Height - 125;
        }
        ...
        private void inicializarParametros(ReportParameter[] parametros)
        {
            ...
            parametros[17] = new ReportParameter("descripcion", "Conteo Vehicular por Día, Hora y Clase");
